 ...
 * @brief Change from cpp mavlink headers to c mavlink headers
 * 
 * Similar to https://github.com/mavlink/mavros/blob/master/mavros/src/lib/uas_data.cpp
 * 
...
UAS::UAS() :
	...
	// tsync_mode(UAS::timesync_mode::PASSTHROUGH), // modified by Peixuan Shu to use mavlink message time for ROS message timestamp headers
...
static uint64_t get_default_caps(MAV_AUTOPILOT ap_type)
{
...
void UAS::update_capabilities(bool known, uint64_t caps)
{
	...
	if (process_cb_queue) {
		...
		for (auto &cb : capabilities_cb_vec) {
			cb(static_cast<MAV_PROTOCOL_CAPABILITY>(caps));
		}
	}
}
