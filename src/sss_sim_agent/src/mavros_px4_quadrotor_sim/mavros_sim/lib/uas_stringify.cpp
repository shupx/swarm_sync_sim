 ...
 * 
 * Similar to https://github.com/mavlink/mavros/blob/master/mavros/src/lib/uas_stringify.cpp
 * 
...
/**
 * @brief Make printf-formatted std::string (from libmavconn/include/mavconn/thread_utils.h)
 *
 */
...
static inline std::string str_base_mode(int base_mode) {
	return format("MODE(0x%2X)", base_mode);
}

static std::string str_custom_mode(uint32_t custom_mode) {
	return format("CMODE(%u)", custom_mode);
}
...
static inline bool is_apm_copter(MAV_TYPE type)
{
